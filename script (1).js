/**
 * ═══════════════════════════════════════════════════════
 *  SCRIPT.JS — Alpha Korner | Logique d'interaction
 *
 *  Fonctions principales :
 *  - initNavLinks()     : routeur data-target → config.js NAV_LINKS
 *  - showPage(page)     : SPA router (home | services | demo | audit)
 *  - openCallModal()    : ouvre la modal de contact téléphonique
 *  - showDemo()         : switcher des démonstrations pills
 *  - toggleFaq()        : accordéon FAQ
 *  - startAuditSPA()    : lance l'audit PageSpeed + fallback IA
 *  - initServicesPage() : animations des service cards
 *  - animateCounter()   : compteur animé chiffres clés
 *  - addRipple()        : effet ripple sur boutons
 *
 *  Dépendances :
...
/* ════════════════════════════════════════════════
   ROUTEUR data-target → config.js NAV_LINKS
   Écoute tous les boutons portant [data-target].
   Redirige vers l'URL correspondante dans NAV_LINKS.
════════════════════════════════════════════════ */
function initNavLinks() {
  document.querySelectorAll('[data-target]').forEach(function(el) {
    el.addEventListener('click', function(e) {
      ...
      const url = NAV_LINKS[key];
      if (!url) return;
      // Liens internes SPA
      const spaKeys = ['home', 'services', 'demo', 'audit'];
      if (spaKeys.includes(key)) {
        showPage(key);
        window.scrollTo(0, 0);
...
function showPage(page, anchor) {
  ...
  if (page === 'services') {
    ...
    currentPage = 'demo';
    // Update URL for demo
    if (history.pushState) history.pushState({page:'demo'}, '', '/demo');
  } else if (page === 'audit') {
    ...
    window.scrollTo(0, 0);
    currentPage = 'audit';
    if (history.pushState) history.pushState({page:'audit'}, '', '/audit');
    ...
    currentPage = 'home';
    if (history.pushState) history.pushState({page:'home'}, '', '/');
  }
}
...
/* ════════════════════════════════════════════════
   URL ROUTING — audit page on direct load / back button
════════════════════════════════════════════════ */
(function() {
  function routeByUrl() {
    const path = window.location.pathname;
    if (path === '/audit' || path.includes('audit')) {
      showPage('audit');
    } else if (path === '/demo') {
      showPage('demo');
  ...
  // On direct load
  document.addEventListener('DOMContentLoaded', routeByUrl);
...
/* ════════════════════════════════════════════════
   INIT — Point d'entrée global
════════════════════════════════════════════════ */
// Initialise le routeur data-target au chargement du DOM
